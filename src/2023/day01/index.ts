...
const wordTokens = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"] as const;
...
const parse = (input: string, findStringRepresentations: boolean): Array<Array<Digit>> => {
  const lines = new Input(input).asLines().asStrings();
  const tokens = findStringRepresentations ? [...digitTokens, ...wordTokens] : [...digitTokens];
...
const getCalibrationValue = (line: Array<Digit>) => parseInt(line[0] + line[line.length - 1]);
